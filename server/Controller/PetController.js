...
const allPets = async (reqStatus, req, res) => {
  try {
    ...
    console.log(`Fetching pets with status: ${reqStatus}, Found: ${data.length}`); // Debug log
    
    if (data.length > 0) {
      res.status(200).json(data);
    } else {
      res.status(404).json({ error: 'No data found', requestedStatus: reqStatus });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// New function to get ALL pets (for debugging)
const getAllPets = async (req, res) => {
  try {
    const data = await Pet.find({}).sort({ updatedAt: -1 });
    console.log(`Total pets in database: ${data.length}`);
    res.status(200).json(data);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
